...
(function (global) {
  ...
  //   p1 = (视角增益, 色相漂移, **走线疏密阈值**, 遮罩)
  //   p2 = (**留白阈值**, ·, ·, ·)
  //   电路网格 0.028 = 一格占 2.8% 卡高（横向约 24 格、纵向约 36 格）；
  //     原来是 0.040（15×22 格）—— 格子面积砍一半 = **图案密度翻一倍**，
  //     这是用户说的"电路板的图案再复杂一倍"。
  //   走线半宽 0.105 格 → 线宽约 0.6% 卡高（1185px 的卡上约 7px）
  //   疏密阈值 0.12 → 约 7/8 的"车道"有铜
  //   留白阈值 0.32 → 约三分之一的斑块整片没铜（真板子上一大片是空的）
  ...
  //     0.225 → 任何角度最多 45% 的字显形
  //     （原来是 0.100 = 20%，用户要"看得见更多图案"，连着色器里那道硬上限
  //      一起从 0.125 = 25% 抬到 0.225 = 45%，两处必须同步改）
  //   字形密度 0.138 = 一格占 13.8% 卡高（也就是字与字的间隔）
  //   p2 = (金点强度, 金点密度, 金点尺寸, ·)
  //     全图撒的小金点：颜色跟着观察角流动，权重刻意很小 —— 它是辅助层，不抢象形字
  const millennium = (str) => L('millennium',
    [str, 0.225, 0.85, 0.138], [1.10, 0, 0, SEL.ALL], [0.30, 54, 0.14, 0], C.goldLight);
  ...
  const LIST = [
    ...
    },
    {
      ...
      feat: 'TCG 特殊埃及风闪膜',
      render: '埃及文字图案膜铺满整卡（同样一批一批显形，任何角度最多 45%），再压一层淡金工艺 —— 千年闪的"高级版"。',
    ...
    },
    {
      ...
      render: '**金闪打底**（卡图全息 + 金卡名），上面再加一层**电路板**膜，画的是**四层铜**：' +
        '每层的走向场、留白、疏密、线宽都不一样，叠起来互相交叉；走线成组地朝一个方向走、' +
        '走很长一段才折一下（折角多是 **45°**），线的尽头压**过孔**，少数几根是粗一档的' +
        '"电源线"，板上还留着一片片没铜的空地 —— 是一块**板子**。配色是' +
        '**蓝 → 绿 → 黄 → 橙 → 红**的一条**连续渐变**（在色相上插值，不是几档硬切），' +
        '**按到光标的距离**铺开：光标那儿是蓝，往外一路绿、黄、橙，到窗口边缘是几乎看不见的红。' +
    ...
    },
    {
      ...
      render: '**埃及象形字**按格随机拼成的图案膜（象形字是风格化近似，见 README）：一格一个字、' +
        ...
        '整片膜按位置分成许多组，**任何角度最多只有 45% 的字显形**，转动时一批一批地亮起来；' +
