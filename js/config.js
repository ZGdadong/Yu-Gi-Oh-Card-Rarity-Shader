...
(function (global) {
  ...
  const SPEC = [
    ...
    // ---- ④ 区域（卡图窗 / 文字区 / 星数阶数带 / 属性圆）----
    // 前三个矩形原本是烘焙时按每张卡检测出来、写进 js/card-textures.js 的。
    // 但它们本来就是**纯几何**（不依赖图像内容），所以已挪到着色器里现算 ——
    // 现在拖滑条**实时生效**，不用重烤（重烤一次要 35 秒，没法调）。
    // 星数/阶数带与属性圆是**后加的两块**（遮罩码 11 / 12），一开始就在运行时，没有烘焙值。
    ...
    // （**唯一例外**是 attrR：它的单位是"卡高 = 1"而不是 cardUV，见那条 hint。）
    //
    // ⚠️ 前 12 根（三个矩形）的默认值是**手调**出来的，跟烘焙时那份（regionManual 关掉时
    //    用的 js/card-textures.js 的 regions）**已经不一样了** ——
    //    别再假设"只切开关画面不变"。最后 7 根（星带 + 属性圆）只有这一个来源，开关切不动它们。
    { key: 'regionManual', group: '区域', label: '手动区域', type: 'check', def: 1,
      hint: '打开时下面那 12 根矩形滑条（卡图窗外框 / 怪物区 / 效果文字区）生效；关掉就回到每张卡烘焙时写进 js/card-textures.js 的值 —— 那份还停在实测口径，跟现在这版手调默认值不一样，所以只切开关画面就会变。**星数/阶数带与属性圆不受这个开关影响**：它们是后加的区域，没有烘焙值（见各自的 hint）' },
    ...
    // ---- 新加的两块：星数/阶数带、属性圆（遮罩码 11 / 12）----
    // 这两块**故意做成整条带 / 一个圆**，而不是把每一颗星、每一个图标抠出来：
    //   · 星位随召唤方式换边 —— 等级星靠右（同调/融合）、阶数星靠左（超量，
    //     实测 Raidraptor 最左边那颗星从原图 x=33 就开始了），所以带子必须整宽
    //   · 属性图标是**一个圆**（外面一圈金边 + 里面的属性字），用矩形去框会连带
    //     把卡框那圈深色边一起加工，看着就是个方块 —— 所以它走圆掩膜
    // 它们**不进烘焙**（js/bake.js 的 REGIONS 里没有它们），因此「手动区域」那个开关
    // 管不到它们：两种模式都用手调值。实测的像素位置见各自的 hint。
    { key: 'starX0', group: '区域', label: '星数 / 阶数带 · 左', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.0320,
      hint: '整条带子的左沿。默认 0.0320（≈26px，正好是卡片自己的左沿）—— **超量的阶数星是从最左边开始的**，收窄一点就会把排头那几颗星切掉' },
    { key: 'starY0', group: '区域', label: '星数 / 阶数带 · 上', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.1200,
      hint: '默认 0.1200（≈142px）。实测星盘上沿：A-to-Z 153px / Blue-Eyes 150px / 超量那张 146px，留了几像素余量' },
    { key: 'starX1', group: '区域', label: '星数 / 阶数带 · 右', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.9350,
      hint: '整条带子的右沿。默认 0.9350（≈760px）—— 等级星那一行是**右对齐**的（10 颗星到 688px 为止），留出余量免得换卡被切' },
    { key: 'starY1', group: '区域', label: '星数 / 阶数带 · 下', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.1800,
      hint: '默认 0.1800（≈213px）。实测星盘下沿 192~205px；再往下就压到卡图窗外框了（外框上沿 0.1673）' },

    { key: 'attrX', group: '区域', label: '属性圆 · 圆心 x', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.8831,
      hint: '名字后面那个属性图标（光/暗/地/水/炎/风/神）的圆心。默认 0.8831（≈718px）—— 三张卡实测都在 716.5~718.8px' },
    { key: 'attrY', group: '区域', label: '属性圆 · 圆心 y', type: 'range', min: 0, max: 1, step: 0.0001, def: 0.0759,
      hint: '默认 0.0759（≈90px）。实测三张卡：88.8 / 89 / 92.3px' },
    { key: 'attrR', group: '区域', label: '属性圆 · 半径', type: 'range', min: 0, max: 0.12, step: 0.0001, def: 0.0320,
      hint: '单位是**卡高 = 1**（和「卡片圆角」同一套口径），默认 0.0320 ≈ 38px —— 正好盖住图标外面那圈金边。圆按卡片的宽高比算，不会被拉成椭圆' },
