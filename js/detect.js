/*
 * detect.js —— 卡片外沿检测：**唯一的定义**
 *
 * 这一份被三处共用：
 *   · index.html 直接 <script> 引入（运行时加卡的烘焙要用它）
 *   · tools/embed-card.mjs  通过 tools/lib/detect.mjs 读文件文本、在页面里 eval
 *   · tools/verify.mjs      同上
 * 所以这里**必须是经典脚本**（不能有 import/export）—— 页面 file:// 下加载不了 ES module。
 * 改了一处三边都跟着变，不会出现"verify 测的是另一套算法"这种情况。
 *
 * 算法：
...
(function (global) {
  ...
  global.CardDetect = { detectCardRect: detectCardRect, CARD_ASPECT: CARD_ASPECT };
  // tools/*.mjs 是 `eval(DETECT_SRC)` 之后**直接**调 detectCardRect(...) 的，
  // 所以这里再挂一个同名全局，省得去改那两个工具的调用点。
  global.detectCardRect = detectCardRect;
