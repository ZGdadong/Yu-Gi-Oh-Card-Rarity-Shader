...
| **图案** | `kc` / `millennium` / `stamp` | 电路板膜（**四层铜**：成组的长走线 + 45° 折角 + 过孔 + 留白，**鼠标附近那一块显形，窗口 ≈45%、对焦最亮往外渐变**，配色按到光标的距离在蓝→绿→黄→橙→红之间连续渐变）/ 埃及象形字图案膜（另加全图小金点，一个角度最多显形 45%）/ 20th·25th 水印 —— 见 `docs/rarity-shaders.md` §4 |
...
| 区域 | 说明 | 占比 |
| --- | --- | --- |
| **卡名笔画** | 卡名带里**按暗度抠出来**再膨胀 2px —— 不猜字形位置，换任何字体都跟得住 | 1.85% |
| **卡图** | 插画本身（不含四周深色框） | 40.7% |
| **效果框** | 含 ATK/DEF 带 | 18.9% |
| **卡框** | 整卡 − 卡图窗外框 − 效果框（着色器按矩形现算） | 37.2% |
| 卡图外环 | 卡图窗外框 − 卡图内容 | 3.2% |
| **星数 / 阶数带** | 名带与卡图窗之间那一条，**整宽**（等级星靠右、超量阶数星靠左，一条带子两种都盖得住） | 遮罩码 11 |
| **属性圆** | 名字后面那个圆（光/暗/地/水/炎/风/神），**走圆掩膜**不当方块框 | 遮罩码 12 |
...
### `tools/effects.mjs` —— 43 + 17 + 60 + 23 全部实测
...
>
> 「区域」那 20 根滑条以前一直被这份扫描判成"没接通"，那是**工况不对**：几何参数只有在
> "加工那一块的那一层"上才看得出来，而合成卡片上后面铺满整卡的膜把前面盖住了。
> 现在给每根配一层"只留它、并且它的遮罩就选那一块"的膜（星带 → 11、属性圆 → 12、
> 卡图外框 → 7、插画 → 1、效果框 → 4），于是全部实测生效。
