...
const CRAFT_DOC = {
  ...
  parallel: ['整卡极细的**不规则拉丝**膜（不是等距线栅）+ 沿垂直方向的缓变色带，倾斜时整片彩虹一起扫过去（爆闪）', '强度, 拉丝密度, 拉丝锐度, 暗部增强', '色带频率, 视角增益, 线角度, **遮罩**', '·'],
  ...
  kc: ['**电路板膜**（**四层铜**：每层的走向场、留白、疏密、线宽都不一样，叠起来互相交叉；走线成组地朝一个方向走 + 45° 折角 + 过孔 + 粗"电源线" + 留白空地）；配色是**蓝 → 绿 → 黄 → 橙 → 红**的一条**连续渐变**（色相上插值，不是几档硬切），**按到光标的距离**铺开；**只有鼠标附近那一块显形** —— 以光标为圆心、半径 0.31 卡高的软斑（面积 ≈ 卡面 45%），**对焦那圈最亮、往外一路渐变**', '强度, 电路网格尺寸, 走线半宽, **可见半径**', '视角增益, 色相漂移, **走线疏密阈值**, **遮罩**', '**留白阈值**, ·, ·, ·'],
  millennium: ['埃及象形字按格随机拼；一格一个字、随机镜像，字自带彩色折射。整片按波带分组，**任何角度最多 45% 的字显形**；另在整卡撒一层跟着角度变色的小金点（辅助层，很淡）。字与字之间**没有竖线**（实卡是图案膜不是线栅膜）', '强度, 可见窗口半宽, 字形强度, 字形密度', '视角增益, ·, ·, **遮罩**', '金点强度, 金点密度, 金点尺寸, ·'],
